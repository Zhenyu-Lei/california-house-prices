...
def log_rmse(net, loss, features, labels, batch_size):
    # 把模型输出的值限制在1和inf之间，inf代表无穷大（infinity的缩写）
    data_iter = load_array((features, labels), batch_size)
    rmse = 0.0
    for X, y in data_iter:
        clipped_preds = torch.clamp(net(X), 1, float('inf'))
        rmse_part = torch.sqrt(loss(torch.log(clipped_preds), torch.log(y)))
        rmse += rmse_part.item()
    return rmse
...
def train(net, loss, train_features, train_labels, test_features, test_labels,
    ...
    for epoch in range(num_epochs):
        for X, y in train_iter:
            ...
            l.backward()
            optimizer.step()
        train_ls.append(log_rmse(net, loss, train_features, train_labels, batch_size))
        if test_labels is not None:
            test_ls.append(log_rmse(net, loss, test_features, test_labels, batch_size))
